...
for i in range(token_size):
    ...
    for j in range(len(dictionary)):
        if jf.damerau_levenshtein_distance(misspell[i], dictionary[j]) <= distance_limit:
            edit_distance = jf.damerau_levenshtein_distance(misspell[i], dictionary[j])
            matching.append([edit_distance, dictionary[j]])
    matching.sort()
